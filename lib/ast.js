...
exports.Block = Block = (function(superclass){
  ...
  prototype.compileRoot = function(options){
    var o, bare, ref$, ref1$, prefix, ref2$, code;
    o = (import$({
      level: LEVEL_TOP,
    ...
    if (/^\s*(?:[/#]|javascript:)/.test((ref1$ = this.lines[0]) != null ? ref1$.code : void 8)) {
      prefix = this.lines.shift().code + '\n';
    }
    if ((ref2$ = o.eval, delete o.eval, ref2$) && this.chomp().lines.length) {
      if (bare) {
...
exports.Chain = Chain = (function(superclass){
  ...
  prototype.cacheReference = function(o){
    ...
    return [Chain(base).add(name), Chain(bref || base.head, [nref || name])];
  };
  ...
  prototype.unfoldSoak = function(o){
    var that, ref$, i, ref1$, len$, node, ref2$, bust, test, ref3$, ref4$, ref5$, ref6$, ref7$;
    ...
    for (i = 0, len$ = (ref1$ = this.tails).length; i < len$; ++i) {
      ...
      if (ref2$ = node.soak, delete node.soak, ref2$) {
        ...
        test = node instanceof Call
          ? (ref3$ = bust.cacheReference(o), test = ref3$[0], this.head = ref3$[1], JS("typeof " + test.compile(o, LEVEL_OP) + " == 'function'"))
          : (i && node.assign
            ? (ref4$ = bust.cacheReference(o), test = ref4$[0], bust = ref4$[1], this.head = bust.head, (ref5$ = this.tails).unshift.apply(ref5$, bust.tails))
            : (ref6$ = bust.unwrap().cache(o), test = ref6$[0], this.head = ref6$[1]), Existence(test));
        return ref7$ = If(test, this), ref7$.soak = true, ref7$.cond = this.cond, ref7$['void'] = this['void'], ref7$;
      }
    }
  };
...
exports.Assign = Assign = (function(superclass){
  ...
  prototype.compileNode = function(o){
    var left, ref$, i$, ref1$, len$, op, ref2$, right, minmax, lefs, ref3$, reft, ref4$, ref, ref5$, j$, x0$, len1$, code, name, lvar, that, res;
  ...
  prototype.rendObj = function(o, nodes, rite){
    var i$, len$, node, splat, logic, ref$, key, val, rcache, ref1$, results$ = [];
    for (i$ = 0, len$ = nodes.length; i$ < len$; ++i$) {
      node = nodes[i$];
      ...
      results$.push((ref1$ = clone$(this), ref1$.left = node, ref1$.right = val, ref1$['void'] = true, ref1$).compile(o, LEVEL_PAREN));
    }
    return results$;
  };
  return Assign;
  ...
  prototype.compileNode = function(o){
    ...
    if (this.from) {
      ...
      if (this.item || this.object && this.own) {
        ref2$ = this.source.compileLoopReference(o, 'ref', !this.object), svar = ref2$[0], srcPart = ref2$[1];
      } else {
