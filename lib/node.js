...
module.exports = function(Coco){
  ...
  return require.extensions['.co'] = function(module, filename){
    var js, _;
    _ = fs.readFileSync(filename, 'utf8');
    js = Coco.compile(_, {
      filename: filename,
      bare: true,
      lineno: true
...
function hackTrace(error, js, filename){
  var traces, colineno, i, trace, index, lno, end, length, lines, n, no, line, _len, _ref;
  ...
  for (i = 0, _len = traces.length; i < _len; ++i) {
    ...
    for (n = 1 > (_ref = lno - 4) ? 1 : _ref; n <= end; ++n) {
      _ref = colineno.exec(lines[n - 1] || ''), no = _ref[1] || '', line = _ref[2];
      traces[i] += "\n" + ('     ' + no).slice(-5) + '|+'.charAt(n === lno) + " " + line;
    }
  }
