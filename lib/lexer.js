var string, TABS, unlines, enlines, enslash, reslash, camelize, deheregex, character, KEYWORDS_SHARED, KEYWORDS_UNUSED, KEYWORDS, ID, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, BSTOKEN, JSTOKEN, NUMBER, NUMBER_OMIT, REGEX, LASTDENT, INLINEDENT, NONASCII, OPENERS, CLOSERS, INVERSES, CHAIN, ARG, BLOCK_USERS, slice$ = [].slice;
...
exports.pp = function(token){
  ...
  if (tag === 'INDENT' || tag === 'DEDENT') {
    return tag.toLowerCase() + " by " + val;
  }
  if (/^\s+/.test(val)) {
    return tag;
  }
...
exports.doLine = function(code, index){
  var ref$, input, tabs, length, last, delta, tag, val, ref1$;
...
exports.doLiteral = function(code, index){
  var sym, tag, val, ref$, ref1$, that, ref2$, up, ref3$;
  ...
  case ':':
    ...
    break;
  case '*':
    if (that = ((ref2$ = this.last[0]) === 'NEWLINE' || ref2$ === 'INDENT' || ref2$ === 'THEN') && this.doInlinedent(code, index + 1, 'list')) {
      return that;
    }
  ...
  default:
    if ('=' === val.slice(-1)) {
      ...
      } else if ((val === '+=' || val === '-=' || val === '^=') && !able(this.tokens) && ((ref3$ = this.last[0]) !== '+-' && ref3$ !== '^' && ref3$ !== 'UNARY' && ref3$ !== 'LABEL')) {
...
exports.doInlinedent = function(code, index, list){
  var d;
  if (!(d = (INLINEDENT.lastIndex = index, INLINEDENT).exec(code)[0])) {
    return 0;
  }
  this.dentlint(d.slice(0, -1));
  list && this.tokens.push(['LITERAL', 'void', this.line], ['ASSIGN', '=', this.line]);
  this.indent(index + d.length - this.dent - 2 - code.lastIndexOf('\n', index));
  return d.length;
};
...
exports.dentlint = function(dent){
  var that;
  if (that = dent && (this.emender || (this.emender = RegExp('[^' + dent.charAt(0) + ']'))).exec(dent)) {
...
deheregex = replacer(/\s+(?:#.*)?|(\\[\s\S])/g, function(arg$, bs){
  bs || (bs = '');
  if ('\n' === bs.charAt(1)) {
    return '\\n';
  } else {
