...
exports.doLiteral = function(code, index){
  ...
  if (tag === ',' || tag === '=>' || tag === 'DOT' || tag === 'LOGIC' || tag === 'COMPARE' || tag === 'MATH' || tag === 'IMPORT' || tag === 'SOFTIMPORT' || tag === 'SHIFT' || tag === 'BITWISE') {
    this.unline();
  }
  this.token(tag, val);
  return sym.length;
...
function addImplicitParentheses(tokens){
  ...
  function ok(token, i){
    ...
    case 'INDENT':
      ...
      return (__ref = pre[0]) !== '{' && __ref !== '[' && __ref !== ',' && __ref !== '->' && __ref !== ':' && __ref !== 'ELSE' && __ref !== 'ASSIGN' && __ref !== 'SOFTIMPORT' && __ref !== 'IMPORT' && __ref !== 'UNARY' && __ref !== 'DEFAULT' && __ref !== 'TRY' && __ref !== 'CATCH' && __ref !== 'FINALLY' && __ref !== 'HURL' && __ref !== 'DO';
    case 'WHILE':
      if (token.done) {
        return false;
      }
...
KEYWORDS_SHARED = ['true', 'false', 'null', 'this', 'void', 'super', 'return', 'throw', 'break', 'continue', 'if', 'else', 'for', 'while', 'switch', 'case', 'default', 'try', 'catch', 'finally', 'class', 'extends', 'new', 'do', 'delete', 'typeof', 'in', 'instanceof', 'import', 'softimport', 'function', 'let', 'with', 'debugger', 'export'];
